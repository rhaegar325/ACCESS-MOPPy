...
class ACCESS_ESM_CMORiser:
    ...
    def to_iris(self):
        ...
        # Ocean grids store latitude/longitude as 2D data variables (j, i).
        # xarray set_coords makes ncdata convert them to iris AuxCoords
        # instead of separate Cubes.
        aux_vars = [
            "latitude",
            "longitude",
            "vertices_latitude",
        ...
        # standardize_missing_values() sets data to 1e20 for NetCDF output,
        # but iris needs NaN (or masked arrays) to represent missing data.
        if cmor_name in ds.data_vars:
            ...
            if fill_val is not None:
                try:
                    ...
                    ds[cmor_name] = ds[cmor_name].where(
                        ds[cmor_name] != fill_val
                    )
